...
class RegistrationSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if username is None:
            raise serializers.ValidationError(
                {"username": "An username is required to register."}
            )
        if password is None:
            raise serializers.ValidationError(
    ...
    def create(self, validated_data):
        ...
        UserProfile.objects.create(pk=user.pk, user=user)
        return user
...
class LoginSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if username is None:
            raise serializers.ValidationError(
                {"username": "An username is required to log in."}
            )
        if password is None:
            raise serializers.ValidationError(
...
class FriendRequestSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if to_user.friends.filter(pk=from_user.id) and from_user.friends.filter(
            pk=to_user.id
        ):
            raise serializers.ValidationError(
                {"friend_request": "You are already friends with this user"}
            )
        if to_user == from_user:
            raise serializers.ValidationError(
                {"friend_request": "You can not invite yourself"}
            )
        ...
        if existing_request:
            raise serializers.ValidationError(
                {
                    "friend_request": "You have already sent a friend request to this user"
                }
            )
        else:
            existing_request = FriendRequest.objects.filter(
            ...
            if existing_request:
                raise serializers.ValidationError(
                    {"friend_request": "This user already sent a friend request to you"}
                )
        return data
...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ("id", "email", "username", "password", "friend_requests", "friends")
