...
class WhiteWinsTest(TestCase):
    ...
    async def test_game_consumer(self):
        ...
        communicator_1.scope.update({'cookies':{'jwt': None}})
        communicator_2.scope.update({'cookies':{'jwt': None}})
        ...
        if self.game_obj.player_white.username == self.user_1.username:
            ...
            communicator_1.scope['cookies']['jwt'] = self.user_1.token
            communicator_2.scope['cookies']['jwt'] = self.user_2.token
        else:
            communicator_1.scope['user'] = self.user_2
            communicator_2.scope['user'] = self.user_1
            communicator_1.scope['cookies']['jwt'] = self.user_2.token
            communicator_2.scope['cookies']['jwt'] = self.user_1.token
        
        print(communicator_1.scope)
        print(communicator_2.scope)
