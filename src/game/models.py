...
class Game(models.Model):
    ...
    pieces = [None] * 64
    possible_moves = [False] * 64
    attacked_fields = [False] * 64
    halfmoves = 0
    fullmoves = 0
    enpassant_sqr = '-'
    prev_p = None
    ...
    def is_legal(self, p, t):
        ...
        if self.pieces[p].color != self.turn:
            return False, False
    ...
    def castles(self, p):
        if self.pieces[p].moved == False:
            c = self.pieces[p].color
            self.update_attacked_fields(self.turn)
            if c == 'white':
    ...
    def end_move(self, p, t):
        ...
        game_result = self.is_checkmated(self.turn)
