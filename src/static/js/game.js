...
function init_fields() {
	for (var i = 0; i < 64; i++) {
		$("#board").append('<div class="field" id="f'  + i + '" color="none" type="none"></div>');
	}
}
...
function allow_dragging() {
	for (var i = 0; i < 64; i++) {
		...
		else if (piece.attr('color') != player) {
			piece.draggable({
				disabled: true
			});
...
function pawn_promotion() {
	...
	add_promotion_listeners();
}

function add_promotion_listeners() {
...
function piece_pick(e) {
	switch (e.which) {
		case 1:
			if (e.target.getAttribute('color') == player) {
